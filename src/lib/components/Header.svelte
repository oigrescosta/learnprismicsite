...
<header class="p-4 md:p-6">
    <nav class="ma
    mx-auto flex max-w-6xl flex-col justify-between py-2 font-medium text-white md:flex-row md:items-center"
    aria-label="Main">
            ...
            <ul class="grid justify-items-end gap-8">
                {#each settings.data.navigation as item (item.label)}
                    <li>
                        ...
                        {:else}
                                <PrismicLink on:click={close} field={item.link}
        ...
        <ul class="hidden md:flex gap-6">
            ...
            (item.label)}
                <li>
                    ...
                    {:else}
                            <PrismicLink field={item.link} on:click={close} aria-current={isActive(item.link) ? 'page' : undefined}
